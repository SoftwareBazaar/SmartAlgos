...
export function BacktestEngineSection() {
  ...
  const requestSheet = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.includes("@")) {
      toast.error("Enter a valid email");
      return;
    }
    toast.success("Tear sheet requested", {
      description: "We'll email an illustrative sample PDF and waitlist you for the live engine.",
    });
    setEmail("");
  };

  return (
    <section id="backtest" className="py-20 border-t border-border bg-secondary-surface/40 scroll-mt-24">
      <div className="max-w-[1400px] mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <p className="text-muted-foreground text-sm mt-2">
            Preview how a rules-based run is reported. Figures below are a sample tear sheet — not a live fill of your
            parameters — so you can see the output format before the engine is generally available.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <form className="space-y-4" onSubmit={run}>
              ...
              >
                {running ? (
                  <>
                    <Loader2 className="h-3.5 w-3.5 animate-spin" /> Running sample…
                  </>
                ) : (
                  "Run sample backtest"
                )}
            ...
            <div className="lg:col-span-2 bg-dominant border border-border rounded-xl p-6 flex flex-col justify-between">
                ...
                <div className="flex justify-between items-center mb-4 gap-2">
                  <span className="text-xs font-mono text-muted-foreground">
                    ...
                    <span className={result ? "text-bull" : "text-gold"}>
                      {result ? "Sample complete" : "Awaiting run"}
                    </span>
                  </span>
              ...
              >
                ...
                <button
                  type="submit"
                  className="sm:w-auto px-4 min-h-12 py-2 bg-secondary-surface hover:bg-card text-gold border border-gold/30 font-bold text-xs uppercase tracking-wider rounded-lg transition-colors cursor-pointer"
                >
                  Email sample PDF
                </button>
              </form>
