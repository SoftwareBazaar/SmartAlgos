    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4 border border-gray-200" onClick={(e) => e.stopPropagation()}>
                <h3 className="text-2xl font-bold mb-6 text-slate-900 text-center">Checkout</h3>

                {/* EA Details */}
                {ea && (
                    <div className="bg-slate-50 p-5 rounded-lg mb-6 border border-slate-100">
                        <p className="text-xs uppercase tracking-wider text-slate-500 font-semibold mb-1">Subscribing to:</p>
                        <p className="text-lg font-bold text-slate-800 line-clamp-2">{ea.name}</p>

                        <div className="flex justify-between mt-4 pb-2 border-b border-slate-200">
                            <span className="text-sm text-slate-600">Subscription Type:</span>
                            <span className="text-sm font-bold text-slate-800 capitalize">{subscriptionType}</span>
                        </div>

                        <div className="flex justify-between mt-3">
                            <span className="text-sm text-slate-600">Total Amount:</span>
                            <span className="text-2xl font-extra-bold text-emerald-600">
                ...
                {/* Error Message */}
                {error && (
                    <div className="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3 rounded-lg mb-6 text-sm flex items-start gap-2">
                ...
                {!paymentConfig.publicKey && !isProcessing && (
                    <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg mb-6 text-xs italic">
                ...
                {isProcessing && !paymentConfig.reference && (
                    <div className="flex flex-col items-center justify-center py-6">
                        ...
                        <span className="text-slate-600 font-medium text-sm">Initializing payment...</span>
                    </div>
                )}
