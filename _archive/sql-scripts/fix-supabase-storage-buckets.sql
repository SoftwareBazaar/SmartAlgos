...
-- Create utilities bucket for utility images and files
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'utilities',
  'utilities',
...
DROP POLICY IF EXISTS "Public can view utilities" ON storage.objects;
...
DROP POLICY IF EXISTS "Authenticated users can upload utilities" ON storage.objects;
...
DROP POLICY IF EXISTS "Authenticated users can update utilities" ON storage.objects;
...
DROP POLICY IF EXISTS "Authenticated users can delete utilities" ON storage.objects;
...
CREATE POLICY "Public can view utilities"
ON storage.objects FOR SELECT
TO public
...
CREATE POLICY "Authenticated users can upload utilities"
ON storage.objects FOR INSERT
TO authenticated
...
CREATE POLICY "Authenticated users can update utilities"
ON storage.objects FOR UPDATE
TO authenticated
USING (bucket_id = 'utilities')
WITH CHECK (bucket_id = 'utilities');
...
CREATE POLICY "Authenticated users can delete utilities"
ON storage.objects FOR DELETE
TO authenticated
USING (bucket_id = 'utilities');
...
WHERE name IN ('ea-images', 'ea-screenshots', 'ea-files', 'utilities')
ORDER BY name;
