...
import { PremiumStat } from "@/components/premium/premium-stat";
    ...
    <div className="min-h-screen bg-background text-foreground">
      <nav className="fixed top-0 inset-x-0 z-50 glass-nav">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/consultation" className="hidden sm:inline-flex items-center gap-1.5 rounded-sm bg-primary px-4 py-2 text-xs font-semibold uppercase tracking-wider text-primary-foreground hover:bg-gold-soft transition duration-200 cursor-pointer">
      ...
      <section className="relative min-h-[92vh] pt-32 pb-20 overflow-hidden flex items-center bg-dominant">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 w-full">
          <MotionHeroText className="max-w-4xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-gold/25 bg-secondary-surface/80 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-gold mb-7 premium-shimmer">
      ...
      <section className="border-y border-border bg-secondary-surface/80">
        <div className="max-w-[1400px] mx-auto px-6 py-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
          <PremiumStat label="Research Notes" value={researchOverview.notesPublished} />
          <PremiumStat label="Strategy Studies" value={researchOverview.studiesCompleted} />
          <PremiumStat label="Live Strategies" value={strategyOverview.live} />
          <PremiumStat label="Markets Covered" value={researchOverview.marketsCovered} />
        </div>
      </section>
      ...
      <section className="py-20 border-t border-border bg-secondary-surface/50">
        <div className="max-w-[1400px] mx-auto px-6">
          ...
          <PremiumBento
            items={[
              { icon: FileText, title: "Research", items: ["Free previews", "Full reports — $10", "Methodology & notebooks"], link: "/research" },
              { icon: FlaskConical, title: "Strategies", items: ["Live models — $149.99 retail", "Institutional — $499.99", "Third-party verification"], link: "/strategies" },
              { icon: Brain, title: "Advisory", items: [`Consultation — ${formatUsd(PRICING.consultation)}`, "Strategy review", "System design"], link: "/consultation" },
            ]}
          />
        </div>
      </section>
      ...
      <section className="py-16 border-t border-border">
        <div className="max-w-[1400px] mx-auto px-6 text-center">
          ...
          <Link to="/performance" className="mt-6 inline-flex items-center gap-2 text-sm text-gold hover:underline cursor-pointer">
          ...
          <div className="grid md:grid-cols-3 gap-10 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                {nav.map((item) => (
                  <li key={item.to}><Link to={item.to} className="hover:text-gold transition cursor-pointer">{item.label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                <Link to="/about" hash="roadmap" className="hover:text-gold transition cursor-pointer">View our roadmap</Link> — subscriber portal, Collective2 signals, and live performance API.
              </p>
            </div>
          </div>
